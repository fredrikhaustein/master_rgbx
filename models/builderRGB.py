...
class EncoderDecoder_RGB(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d):
        ...
        if cfg.backbone == 'swin_s':
            ...
            from .encoders.dual_segformer_RGB import mit_b0 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        else:
            logger.info('Using backbone: Segformer-B2')
            from .encoders.dual_segformer import mit_b2 as backbone
    ...
    def encode_decode(self, rgb):
        ...
        orisize = rgb.shape  # Original size of the input image
        x = self.backbone(rgb)  # Use the backbone network to encode RGB image
        out = self.decode_head.forward_rgb(x)  # Decode the features into a segmentation map
        out = F.interpolate(out, size=orisize[2:], mode='bilinear', align_corners=False)  # Resize output to match input size

        # If an auxiliary head is used, process its output in the same way
        if self.aux_head:
            aux_fm = self.aux_head(x[self.aux_index])  # Assume x is a list/tuple of feature maps and aux_index is predefined
            aux_fm = F.interpolate(aux_fm, size=orisize[2:], mode='bilinear', align_corners=False)
            return out, aux_fm  # Return main and auxiliary outputs

        return out  # Return only the main output if no auxiliary head is used
